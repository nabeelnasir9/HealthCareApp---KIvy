...
diagrams = {'Headache': "", 'Flashes of Light': "", 'Anisicoria': "", 'RedEye': ""}
histories = []
...
class MainWidget(BoxLayout):
    ...
    def showing_history(self):
        try:
                ...
                if date == "1":
                    ...
                    if lineHistory.find("?") >= 0:
                        ...
                        Content = Content + line.replace("%",
                                                         "[font=EduTASBeginner-Regular][size=15sp] %[/size][/font]")
            ...
            images = {"Headache": "icon_folder/Headache.png", "Flashes of Light": "icon_folder/BlueEye.png",
                      "Anisicoria": "icon_folder/PupilEye2.png", "RedEye": "icon_folder/RedEye.png"}
    ...
    def cancelTest(self, instance):
        global age, name, percentages, percentages2, percentages3, percentages4
        age = 0
        name = ''
        ...
        if instance == 'Headache':
            ...
            self.headaches_list = []
            self.headaches_diag = []
            self.ids.headache_question.text = "Where are the headaches?"
            self.ids.headache_btn1.text = "sides/back"
            self.ids.headache_btn2.text = "front"
            diagrams['Headache'] = ""
        elif instance == 'Flashes of Light':
            percentages2 = ''
            self.flashes_list = []
            self.flashes_diag = []
            self.ids.flashes_question.text = "Are the flashes in one eye or both?"
            self.ids.flashes_btn1.text = "one eye"
            self.ids.flashes_btn2.text = "both"
            diagrams['Flashes of Light'] = ""
        elif instance == 'Anisicoria':
            percentages3 = ''
            self.anisicoria_list = []
            self.anisicoria_diag = []
            self.ids.anisicoria_question.text = "How long have you had this for?"
            self.ids.anisicoria_btn1.text = "New occurence"
            self.ids.anisicoria_btn2.text = "Longstanding"
            diagrams['Anisicoria'] = ""
        elif instance == 'RedEye':
            percentages4 = ''
            self.redeye_list = []
            self.redeye_diag = []
            self.ids.redeye_question.text = "Is it recent or longstanding?"
            self.ids.redeye_btn1.text = "Recent"
            self.ids.redeye_btn2.text = "Longstanding"
            diagrams['RedEye'] = ""
    ...
    def switching_to_anisicoria_screen(self):
        self.current_context = 'Anisocoria'  # Set context
        self.ids.sm.current = "anisicoria_screen"
        self.ids.sm.transition.direction = 'left'

    def switching_to_redeye_screen(self):
        self.current_context = 'RedEye'  # Set context
        self.ids.sm.current = "redeye_screen"
        self.ids.sm.transition.direction = 'left'
        ...
        try:
            ...
            for user in users.each():
                if str(user.key()) == self.username:
                    ...
                    self.ids.account_phone_number_edit.text = self.ids.account_phone_number.text = info_dict[
                        "Phone Number"]
                    self.ids.account_email_address_edit.text = self.ids.account_email_address.text = info_dict[
                        "Email Address"]
                    ...
                    try:
                        self.ids.profile_image_placeholder_edit.source = self.ids.profile_image_placeholder.source = \
                        info_dict["Profile_image"]
                    except Exception as e:
                        print("Image Load Failed!")
                        self.ids.profile_image_placeholder.source = "icon_folder/Account.png"
                    break
            self.headaches_list = []
            self.headaches_diag = []
            self.flashes_list = []
    ...
    def registering(self):
        ...
        if password == confirm_password:
            information = {
                ...
                "City": self.city,
                "Password": password,
                "Profile_image": "icon_folder/Account.png"
            }
    ...
    def main_logic(self, btn_instance_id, label_instance_id, instance):
        print(f"main_logic called with: {btn_instance_id}, {label_instance_id}, {instance}")
        global age, name, percentages, percentages2, percentages3, percentages4
        ...
        if question_text == "Where are the headaches?" and btn_text == "sides/back":
            ...
            print('sides/back')
            self.headaches_list = []
            self.headaches_diag.extend(["Migraine"])
            print(self.headaches_diag)

        elif question_text == "Where are the headaches?" and btn_text == "front":
            self.ids.headache_question.text = "Did this headache come on suddenly or gradually?"
            self.ids.headache_btn1.text = "Suddenly"
            self.ids.headache_btn2.text = "Gradually"
            ...
            self.ids.headache_question.text = "Does this last for seconds, minutes or up to an hour?"
            self.ids.headache_btn1.text = "Seconds"
            self.ids.headache_btn2.text = "Minutes"
            self.ids.headache_btn3.text = "Up to an hour"
            self.ids.headache_btn3.disabled = False

            print('suddenly')
            self.headaches_diag.extend(["Anything"])
            print(self.headaches_diag)
        ...
        # Transient
        elif question_text == "Does this occur in transient episodes? if YES state whether seconds,or up to an hour?" and btn_text == "Seconds" and instance == "Headache":
        ...
        elif question_text == "Does this occur in transient episodes? if YES state whether seconds,or up to an hour?" and btn_text == "Hours" and instance == "Headache":
            self.ids.headache_question.text = "Are the headaches recurrent?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
            self.ids.headache_btn3.disabled = True
        ...
        elif question_text == "Does this occur in transient episodes? if YES state whether seconds,or up to an hour?" and btn_text == "No" and instance == "Headache":
            self.ids.headache_question.text = "Are the headaches recurrent?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
            self.ids.headache_btn3.disabled = True
        ...
        # Recurrent
        elif question_text == "Are the headaches recurrent?" and btn_text == "Yes" and instance == "Headache":
        ...
        elif question_text == "Are the headaches recurrent?" and btn_text == "No" and instance == "Headache":
            self.ids.headache_question.text = "Is Vision Loss present?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
        ...
        # Vision Loss
        # True
        elif question_text == "Is Vision Loss present?" and btn_text == "Yes" and instance == "Headache":
        ...
        elif question_text == "Is Vision loss in central, peri, or shadow like?" and btn_text == "Central" and instance == "Headache":
        ...
        elif question_text == "Is Vision loss in central, peri, or shadow like?" and btn_text == "Peri" and instance == "Headache":
            self.ids.headache_question.text = "Do you have any of the following: unusual headache, tender scalp, jaw ache when chewing, ear or neck pain, weight loss, fatigue, muscle aches?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
            self.ids.headache_btn3.disabled = True
        ...
        elif question_text == "Is Vision loss in central, peri, or shadow like?" and btn_text == "Shadow" and instance == "Headache":
            self.ids.headache_question.text = "Do you have any of the following: unusual headache, tender scalp, jaw ache when chewing, ear or neck pain, weight loss, fatigue, muscle aches?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
            self.ids.headache_btn3.disabled = True
        ...
        # False
        elif question_text == "Is Vision Loss present?" and btn_text == "No" and instance == "Headache":
            self.ids.headache_question.text = "Do you have any of the following: unusual headache, tender scalp, jaw ache when chewing, ear or neck pain, weight loss, fatigue, muscle aches?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
        ...
        # List of Diseases
        elif question_text == "Do you have any of the following: unusual headache, tender scalp, jaw ache when chewing, ear or neck pain, weight loss, fatigue, muscle aches?" and btn_text == "Yes" and instance == "Headache":
        ...
        elif question_text == "Do you have any of the following: unusual headache, tender scalp, jaw ache when chewing, ear or neck pain, weight loss, fatigue, muscle aches?" and btn_text == "No" and instance == "Headache":
            self.ids.headache_question.text = "Do you have Eye pain?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
        ...
        # Eye Pain
        elif question_text == "Do you have Eye pain?" and btn_text == "Yes" and instance == "Headache":
        ...
        elif question_text == "Is pain mild, moderate to severe, or shooting.?" and btn_text == "Mild" and instance == "Headache":
        ...
        elif question_text == "Is pain mild, moderate to severe, or shooting.?" and btn_text == "Moderate" and instance == "Headache":
            self.ids.headache_question.text = "Do you have any head/neck pain?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
            self.ids.headache_btn3.disabled = True
        ...
        elif question_text == "Is pain mild, moderate to severe, or shooting.?" and btn_text == "Shooting" and instance == "Headache":
            self.ids.headache_question.text = "Do you have any head/neck pain?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
            self.ids.headache_btn3.disabled = True
        ...
        elif question_text == "Do you have any head/neck pain?" and btn_text == "Yes" and instance == "Headache":
        ...
        elif question_text == "Do you have any head/neck pain?" and btn_text == "No" and instance == "Headache":
            self.ids.headache_question.text = "Is eye pain worse when moving your eye?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
        ...
        elif question_text == "Is eye pain worse when moving your eye?" and btn_text == "Yes" and instance == "Headache":
        ...
        elif question_text == "Is eye pain worse when moving your eye?" and btn_text == "No" and instance == "Headache":
            self.ids.headache_question.text = "Are the headaches worse when lying down?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
        ...
        elif question_text == "Do you have Eye pain?" and btn_text == "No" and instance == "Headache":
            self.ids.headache_question.text = "Are the headaches worse when lying down?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
        ...
        elif question_text == "Are the headaches worse when lying down?" and btn_text == "Yes" and instance == "Headache":
        ...
        elif question_text == "Are the headaches worse when lying down?" and btn_text == "No" and instance == "Headache":
            self.ids.headache_question.text = "Do the headaches come with aura?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
        ...
        # Aura
        elif question_text == "Do the headaches come with aura?" and btn_text == "Yes" and instance == "Headache":
        ...
        elif question_text == "Do the headaches come with aura?" and btn_text == "No" and instance == "Headache":
            self.ids.headache_question.text = "Do you have any of the following: vertigo, limb weakness or numbness?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
        ...
        # Vertigo, etc
        elif question_text == "Do you have any of the following: vertigo, limb weakness or numbness?" and btn_text == "Yes" and instance == "Headache":
        ...
        elif question_text == "Do you have any of the following: vertigo, limb weakness or numbness?" and btn_text == "No" and instance == "Headache":
            self.ids.headache_question.text = "Do you ever see flashes of light in your vision?"
            self.ids.headache_btn1.text = "Yes"
            self.ids.headache_btn2.text = "No"
        ...
        ###############################################Flashes#####################################################
            ...
            self.ids.sm.current = "result_screen"
        #########################################Anisicoria##########################################################
        elif question_text == "Is it recent or longstanding?" and btn_text == "Recent" and instance == "Anisicoria":
        ...
        elif question_text == "Is it recent or longstanding?" and btn_text == "Longstanding" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Is there any vision loss?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_list = []
            ...
            print(self.anisicoria_diag)
        elif question_text == "Is there any vision loss?" and btn_text == "Yes" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Is there any double vision?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(["Adies", "Uveitis", "Closed AGA"])
            print(self.anisicoria_diag)
        elif question_text == "Is there any vision loss?" and btn_text == "No" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Is there any double vision?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(["physiological", "3rd Nerve Palsy", "Horner Routine", "horner SAME"])
            print(self.anisicoria_diag)
        elif question_text == "Is there any double vision?" and btn_text == "Yes" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Is there any pain?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(["3rd Nerve Palsy"])
            print(self.anisicoria_diag)
        elif question_text == "Is there any double vision?" and btn_text == "No" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Is there any pain?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            ...
            print(self.anisicoria_diag)
        elif question_text == "Is there any pain?" and btn_text == "Yes" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Is there any light sensitivity?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(["Uveitis", "Closed AGA"])
            print(self.anisicoria_diag)
        elif question_text == "Is there any pain?" and btn_text == "No" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Is there any light sensitivity?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(["physiological", "Adies", "3rd Nerve Palsy", "Horner Routine", "horner SAME"])
            print(self.anisicoria_diag)
        elif question_text == "Is there any light sensitivity?" and btn_text == "Yes" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Is there any haloes around lights?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(["Uveitis"])
            print(self.anisicoria_diag)
        elif question_text == "Is there any light sensitivity?" and btn_text == "No" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Is there any haloes around lights?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(
            ...
            print(self.anisicoria_diag)
        elif question_text == "Is there any haloes around lights?" and btn_text == "Yes" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Have you had previous uveitis?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(["Closed AGA"])
            print(self.anisicoria_diag)
        elif question_text == "Is there any haloes around lights?" and btn_text == "No" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Have you had previous uveitis?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(
            ...
            print(self.anisicoria_diag)
        elif question_text == "Have you had previous uveitis?" and btn_text == "Yes" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Do you have diabetes or high blood pressure?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(["Uveitis"])
            print(self.anisicoria_diag)
        elif question_text == "Have you had previous uveitis?" and btn_text == "No" and instance == "Anisicoria":
            self.ids.anisicoria_question.text = "Do you have diabetes or high blood pressure?"
            self.ids.anisicoria_btn1.text = "Yes"
            self.ids.anisicoria_btn2.text = "No"
            self.anisicoria_diag.extend(
                ["physiological", "Adies", "3rd Nerve Palsy", "Horner Routine", "horner SAME", "Closed AGA"])
            print(self.anisicoria_diag)
        elif question_text == "Do you have diabetes or high blood pressure?" and btn_text == "Yes" and instance == "Anisicoria":
            self.anisicoria_diag.extend(["3rd Nerve Palsy"])
            ...
            self.ids.sm.current = "result_screen"
        #########################################RedEye##########################################################
        elif question_text == "Is it recent or longstanding?" and btn_text == "Recent" and instance == "RedEye":
            self.ids.redeye_question.text = "Is there any vision loss?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_list = []
            self.redeye_diag.extend(["horner SAME", "3rd Nerve Palsy", "Uveitis", "Closed AGA"])
            print(self.redeye_diag)
        elif question_text == "Is it recent or longstanding?" and btn_text == "Longstanding" and instance == "RedEye":
            self.ids.redeye_question.text = "Is there any vision loss?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.RedEye_btn2.text = "No"
            self.redeye_list = []
            self.redeye_diag.extend(["physiological", "Adies", "Horner Routine"])
            print(self.redeye_diag)
        elif question_text == "Is there any vision loss?" and btn_text == "Yes" and instance == "RedEye":
            self.ids.redeye_question.text = "Is there any double vision?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(["Adies", "Uveitis", "Closed AGA"])
            print(self.redeye_diag)
        elif question_text == "Is there any vision loss?" and btn_text == "No" and instance == "RedEye":
            self.ids.redeye_question.text = "Is there any double vision?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(["physiological", "3rd Nerve Palsy", "Horner Routine", "horner SAME"])
            print(self.redeye_diag)
        elif question_text == "Is there any double vision?" and btn_text == "Yes" and instance == "RedEye":
            self.ids.redeye_question.text = "Is there any pain?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(["3rd Nerve Palsy"])
            print(self.redeye_diag)
        elif question_text == "Is there any double vision?" and btn_text == "No" and instance == "RedEye":
            self.ids.redeye_question.text = "Is there any pain?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(
                ["physiological", "Adies", "Uveitis", "Closed AGA", "Horner Routine", "horner SAME"])
            print(self.redeye_diag)
        elif question_text == "Is there any pain?" and btn_text == "Yes" and instance == "RedEye":
            self.ids.redeye_question.text = "Is there any light sensitivity?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(["Uveitis", "Closed AGA"])
            print(self.redeye_diag)
        elif question_text == "Is there any pain?" and btn_text == "No" and instance == "RedEye":
            self.ids.redeye_question.text = "Is there any light sensitivity?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(["physiological", "Adies", "3rd Nerve Palsy", "Horner Routine", "horner SAME"])
            print(self.redeye_diag)
        elif question_text == "Is there any light sensitivity?" and btn_text == "Yes" and instance == "RedEye":
            self.ids.redeye_question.text = "Is there any haloes around lights?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(["Uveitis"])
            print(self.redeye_diag)
        elif question_text == "Is there any light sensitivity?" and btn_text == "No" and instance == "RedEye":
            self.ids.redeye_question.text = "Is there any haloes around lights?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(
                ["physiological", "Adies", "3rd Nerve Palsy", "Horner Routine", "horner SAME", "Closed AGA"])
            print(self.redeye_diag)
        elif question_text == "Is there any haloes around lights?" and btn_text == "Yes" and instance == "RedEye":
            self.ids.redeye_question.text = "Have you had previous uveitis?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(["Closed AGA"])
            print(self.redeye_diag)
        elif question_text == "Is there any haloes around lights?" and btn_text == "No" and instance == "RedEye":
            self.ids.redeye_question.text = "Have you had previous uveitis?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(
                ["physiological", "Adies", "3rd Nerve Palsy", "Horner Routine", "horner SAME", "Uveitis"])
            print(self.redeye_diag)
        elif question_text == "Have you had previous uveitis?" and btn_text == "Yes" and instance == "RedEye":
            self.ids.redeye_question.text = "Do you have diabetes or high blood pressure?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(["Uveitis"])
            print(self.redeye_diag)
        elif question_text == "Have you had previous uveitis?" and btn_text == "No" and instance == "RedEye":
            self.ids.redeye_question.text = "Do you have diabetes or high blood pressure?"
            self.ids.redeye_btn1.text = "Yes"
            self.ids.redeye_btn2.text = "No"
            self.redeye_diag.extend(
                ["physiological", "Adies", "3rd Nerve Palsy", "Horner Routine", "horner SAME", "Closed AGA"])
            print(self.redeye_diag)
        elif question_text == "Do you have diabetes or high blood pressure?" and btn_text == "Yes" and instance == "RedEye":
            self.redeye_diag.extend(["3rd Nerve Palsy"])
            ...
            count4 = Counter(self.redeye_diag).items()
            percentages4 = {x: int(float(y) / len(self.redeye_diag) * 100) for x, y in count4}
            if self.ids.lst_placeholder.children:
                self.ids.lst_placeholder.remove_widget(self.ids.lst_placeholder.children[0])
            boxlayout = MDBoxLayout(adaptive_height=True, orientation="vertical", spacing=15)
            self.ids.lst_placeholder.add_widget(boxlayout)
            self.ids.heading.text = "RedEye"
            self.ids.heading_img.source = "icon_folder/RedEye.png"
            history = open(self.username + ".txt", "a+")

            sorted_percentages = sorted(percentages4.items(), key=lambda vari: vari[1], reverse=True)
            for i in sorted_percentages:
                history.write(f"{i[0]}:    {i[1]}%\n")
                label_widget = MDLabel(text=f"{i[0]} - {i[1]} %", size_hint_y=None, height=50, halign="center",
                                       md_bg_color=(212 / 255, 175 / 255, 185 / 255, 0.3), radius=10)
            ...
            self.cancelTest(instance)
            self.ids.sm.current = "result_screen"
        elif question_text == "Do you have diabetes or high blood pressure?" and btn_text == "No" and instance == "RedEye":
            self.redeye_diag.extend(
                ["physiological", "Adies", "Uveitis", "Closed AGA", "Horner Routine", "horner SAME"])
